...
describe Checkout do
  let!(:promotion_2x1) { create(:promotion, quantity:2, discount: 50.0, name: "Nx1") }
  let!(:promotion_x)   { create(:promotion, quantity:3, discount: 10.0, name: "%xN") }
    ...
    context "with promotion 2x1" do
      context "am, ac, am, am, ca" do
        it "charges a product every two products" do
          ...
          subject.scan(am)
          subject.scan(ca)
          expect(subject.total).to eql(22.45)
        end
      end
